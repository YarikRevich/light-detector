    ...
    def handle(device: str, baud_rate: int, type: str):
        ...
        match type:
            case GetInfoCommand.GAIN_TYPE:
                GetInfoCommand.process_get_gain_info(device, baud_rate)

            case GetInfoCommand.INTEGRAL_TIME_TYPE:
                GetInfoCommand.process_get_integral_time_info(device, baud_rate)

            case GetInfoCommand.PROCESSED_REQUESTS_TYPE:
                GetInfoCommand.process_get_processed_requests_info(device, baud_rate)

            case GetInfoCommand.DEVICE_AVAILABLE_TYPE:
                GetInfoCommand.process_get_device_available_info(device, baud_rate)

            case _:
                logging.error("Given info type is not valid.")

    @staticmethod
    def process_get_gain_info(device: str, baud_rate: int):
        """Processes request to retrieve 'gain' metadata info from the device"""

        pass

    @staticmethod
    def process_get_integral_time_info(device: str, baud_rate: int):
        """Processes request to retrieve 'integral_time' metadata info from the device"""

        pass

    @staticmethod
    def process_get_processed_requests_info(device: str, baud_rate: int):
        """Processes request to retrieve 'processed_requests' metadata info from the device"""

        pass

    @staticmethod
    def process_get_device_available_info(device: str, baud_rate: int):
